...
frame change graph_frame
	...
	graph export "$out_dir/C01_$year.png", as(png) replace
	...
	graph export "$out_dir/LI1_$year.png", as(png) replace
	...
	line lost_income_cumul idx if idx < 500, name(LI0_$year, replace) ///
		...
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/LI0_$year.png", as(png) replace
	
	
	gsort -profit_ghati_cal
	gen idx_pg = _n
	cumul idx_pg [w=int(profit_ghati_cal)], gen(profit_ghati_cal_cumul)
	replace profit_ghati_cal_cumul = profit_ghati_cal_cumul * 100
	gsort -profit_ghati_cal_cumul
	line profit_ghati_cal_cumul idx_pg if idx_pg < 100, ///
		title(سهم تجمعی شرکت‌های بزرگ از کل سود قبل از مالیات- سال $year, size(large)) ///
		ytitle(درصد از کل سود قبل از مالیات محاسبه شده, size(medium)) ///
		xtitle(تعداد شرکت, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5)) ///
		name(ProfitC0_$year, replace)
	graph export "$out_dir/PD0_$year.png", as(png) replace
	
	
	gsort -profit_ebrazi
	gen idx_pe = _n
	cumul idx_pe [w=int(profit_ebrazi)], gen(profit_ebrazi_cumul)
	replace profit_ebrazi_cumul = profit_ebrazi_cumul * 100
	gsort -profit_ebrazi_cumul
	line profit_ebrazi_cumul idx_pe if idx_pe < 100, ///
		title(سهم تجمعی شرکت‌های بزرگ از کل سود قبل از مالیات- سال $year, size(large)) ///
		ytitle(درصد از کل سود قبل از مالیات ابرازی, size(medium)) ///
		xtitle(تعداد شرکت, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5)) ///
		name(ProfitC1_$year, replace)
	graph export "$out_dir/PD1_$year.png", as(png) replace
	...
	twoway ///		
		...
		xtitle(صدک شرکت, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/C00_$year.png", as(png) replace	
	...
	line avg_profit_g_percentile percentile_g if !missing(percentile_g), sort  ///
		...
		xtitle(صدک شرکت, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/C001_$year.png", as(png) replace	
...
// 		pie(2, color(255 217 47))	
	...
	graph pie, ///
		over(etr_tag) ///
		by(seg, rows(1) ///
			title("توزیع نرخ موثر مالیاتی شرکت‌ها در صدک های مختلف - سال$year", size(large)) note("")) ///
	...
	graph export "$out_dir/CG13_$year.png", as(png) replace
...
// 	replace is_p100 = 2 if (top200 == 1)
	...
	graph export "$out_dir/CG14_$year.png", as(png) replace
	
	graph pie profit_ghati_cal, ///
		over(is_p100) ///
		title(مجموع سود قبل از مالیات شرکت‌ها, size(large)) ///
		subtitle(, alignment(middle)) ///
		plabel(_all percent, format(%2.0f) color(black) gap(-5)) ///
		plabel(_all name) ///
		line(lcolor(black) lwidth(0.2)) ///
...
// 	graph export "$out_dir/CG14_$year.png", as(png) replace
	...
	graph export "$out_dir/CG141_$year.png", as(png) replace
	...
	graph export "$out_dir/CG15_$year.png", as(png) replace
	...
	graph combine CG14_$year CG141_$year CG15_$year, name(CG14_C_$year, replace) ///
		...
		subtitle("   ") ///
		row(1)
	graph export "$out_dir/CG14_C_$year.png", as(png) replace
	...
	********************
	...
	graph export "$out_dir/CG15_A_$year.png", as(png) replace
	
	label variable tax_ghati "مالیات شرکت"
	label variable lost_income_ebrazi2 "درآمد از دست رفته از محل معافیت‌ها و بخشودگی‌ها"
	...
	graph export "$out_dir/CG15_P_$year.png", as(png) replace
	...
	*********************
	...
	graph export "$out_dir/CG17_$year.png", as(png) replace
	...
	graph export "$out_dir/CG171_$year.png", as(png) replace
	
	graph combine CG171_$year CG17_$year, name(CG17_C_$year, replace)
	graph export "$out_dir/CG17_C_$year.png", as(png) replace
	...
	graph export "$out_dir/CG16_$year.png", as(png) replace
	...
	twoway ///
		...
		xtitle(صدک شرکت, size(medium)) ///
		title(توزیع نرخ موثر مالیات شرکت‌ها در هر صدک -‍ سال $year‍‍, size(large)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5) range(0 1))
	graph export "$out_dir/CG10_$year.png", as(png) replace
	
	preserve
	keep avg_etr_ghati_percentile percentile_g
	duplicates drop
	graph twoway ///
		scatter avg_etr_ghati_percentile percentile_g, m(smplus) mc(gray) || ///
		lowess avg_etr_ghati_percentile percentile_g , sort bwidth(0.15) name(CG09_$year, replace) ///
			ytitle(متوسط نرخ موثر مالیات قطعی شرکت, size(medium)) ///
			xtitle(صدک شرکت, size(medium)) ///
			title(متوسط نرخ موثر مالیاتی در هر صدک -‍ سال $year‍‍, size(large)) ///
			xscale(titlegap(2.5)) yscale(titlegap(1.5)) color(cranberry) ///
			legend(off)
	restore
	graph export "$out_dir/CG09_$year.png", as(png) replace
	...
	graph export "$out_dir/CG03_$year.png", as(png) replace
	...
	graph export "$out_dir/CG01_$year.png", as(png) replace
	...
	hist etr_ghati2 if top1000 == 1, percent bin(26) ///
		title(توزیع نرخ مالیات موثر قطعی ۱۰۰۰ شرکت پرسود -‍ $year, size(large)) ///
		ytitle(درصد, size(medium)) ///
		xtitle(نرخ موثر مالیات قطعی, size(medium)) ///
		color(green%60) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/CG081_$year.png", as(png) replace
	
	hist etr_ghati2 if top500 == 1, percent bin(26) ///
		title(توزیع نرخ مالیات موثر قطعی ۵۰۰ شرکت پرسود -‍ $year, size(large)) ///
		ytitle(درصد, size(medium)) ///
		xtitle(نرخ موثر مالیات قطعی, size(medium)) ///
		color(green%60) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/CG082_$year.png", as(png) replace
	
	hist etr_ghati_s2 if top200 == 1, percent bin(25) ///
		title(توزیع نرخ مالیات موثر قطعی ۲۰۰ شرکت پرسود -‍ $year, size(large)) ///
		ytitle(درصد, size(medium)) ///
		xtitle(نرخ موثر مالیات قطعی, size(medium)) ///
		color(green%60) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/CG083_$year.png", as(png) replace
	...
	twoway ///
		...
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/CG07_$year.png", as(png) replace	
	...
	graph export "$out_dir/CG06_$year.png", as(png) replace
	...
	twoway ///
		...
		ytitle(درصد) xtitle(نرخ موثر مالیات قطعی) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5))
	graph export "$out_dir/CG05_$year.png", as(png) replace
	...
	graph export "$out_dir/CG02_$year.png", as(png) replace
	...
	graph export "$out_dir/CG00_$year.png", as(png) replace
	
	
	gen etr_ghati3 = etr_ghati
	repltace etr_ghati3 = 5 if etr_ghati > 5 & !missing(etr_ghati)
	
	hist etr_ghati3, percent color(red%60) ///
		ylab(, grid) xlab(, grid) ///
		title(توزیع نسبت مالیات قطعی به سود ابرازی شرکت -‍ سال $year, size(large)) ///
		ytitle(درصد از کل شرکت‌ها, size(medium)) ///
		xtitle(نسبت مالیات قطعی به سود ابرازی, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5)) name(CG18_$year, replace) 
	graph export "$out_dir/CG18_$year.png", as(png) replace
	

	gen tax_admin_effort = tax_ghati / tax_ebrazi
	replace tax_admin_effort = 10 if tax_admin_effort > 10 & !missing(tax_admin_effort)
	replace tax_admin_effort = 10 if tax_ghati > 0 & tax_ebrazi == 0 & !missing(tax_ghati)
	hist tax_admin_effort, percent ///
		ylab(, grid) xlab(, grid) ///
		title(نسبت مالیات قطعی به مالیات ابرازی -‍ سال $year, size(large)) ///
		ytitle(درصد از کل شرکت‌ها, size(medium)) ///
		xtitle(نسبت مالیات قطعی به مالیات ابرازی, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5)) name(CG19_$year, replace) 
	graph export "$out_dir/CG19_$year.png", as(png) replace
	
	
	replace tax_admin_effort = 10 if tax_ghati > 0 & tax_ebrazi == 0 & !missing(tax_ghati)
	hist tax_admin_effort , percent ///
		ylab(, grid) xlab(, grid) ///
		title(نسبت مالیات قطعی به مالیات ابرازی با احتساب مالیات ابرازی صفر-‍ سال $year, size(large)) ///
		ytitle(درصد از کل شرکت‌ها, size(medium)) ///
		xtitle(نسبت مالیات قطعی به مالیات ابرازی, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5)) name(CG191_$year, replace) 
	graph export "$out_dir/CG191_$year.png", as(png) replace
	
	
	hist tax_admin_effort [w=int(profit_ghati_cal)], percent bin(50) ///
		ylab(, grid) xlab(, grid) ///
		title(نسبت مالیات قطعی به مالیات ابرازی با احتساب مالیات ابرازی صفر-‍ سال $year, size(large)) ///
		ytitle(سهم از کل سود تخمین زده شده برای شرکت‌ها, size(medium)) ///
		xtitle(نسبت مالیات قطعی به مالیات ابرازی, size(medium)) ///
		xscale(titlegap(2.5)) yscale(titlegap(1.5)) name(CG192_$year, replace) 
	graph export "$out_dir/CG192_$year.png", as(png) replace
